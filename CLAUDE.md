...
Planning and UI design are complete. The catalog job (`catalog/`, `.github/workflows/catalog.yml`) was built from its plan on 2026-09-24 on branch `catalog-job`: 13 tasks, one commit each, 77 tests, a live run against the API, and a whole-branch review whose two Important findings were fixed (`docs/superpowers/reviews/2026-09-24-catalog-branch-fable.md`; its seven minors were fixed later the same day). It is merged to `main`, and its first GitHub run published to `https://flekc09.github.io/tvapp/` on 2026-09-24. The M15 `hasUp` change the same day brought the suite to 78 tests, and the review's seven minors to 87. The app has no code yet. The repo also holds the approved design spec, two implementation plans, a UI design package, and two framework agents. Read the spec before touching anything, and execute only from the plans.
...
- `docs/superpowers/reviews/` — review reports kept verbatim. `2026-09-23-adversarial-opus.md` is the Opus 5.5 adversarial review; its three blockers (B1 float `responseMs`, B2 toolchain re-pin to AGP 9.4.1 / compileSdk 37 / built-in Kotlin 2.2.10, B3 compile errors) were applied to the plans the same day and are annotated inline as "Opus adversarial review 2026-09-23". Its catalog-side majors M12 (geo-blocked media playlist is `unverified`), M13 (no ccTLD guess for generic-use TLDs or redirector hosts; its app-side `countryCounts` half followed with the app majors) and M14 (`force` input, post-deploy `latest.json` check, issue de-duplication) were applied on 2026-09-24 with the same annotation. The five owner decisions that review asked for (M15, M19, m4, m9, m14) were made on 2026-09-24 and are recorded in spec section 11; M15 is applied in the catalog job, M19 removed the backend extras input (missing feeds go upstream to iptv-org), and m4, m9 and m14 are applied to the spec, the UI docs and the app plan. The app-side majors and all remaining minors were applied the same day (majors listed in the app plan's Self-review; minor m23, Node fetch's extra headers, is accepted and recorded in the spec). Every item of that review is now closed.
...
The spec, both plans and the UI package have been through adversarial reviews (engineering, viewer psychology, cold-executor, and a UI review whose blockers, majors and minors were all applied). The catalog plan's tasks were executed verbatim in a scratch directory and pass with type-checking (77 tests then, 78 after the M15 change, 87 after the review minors; every per-task count in the plan matches). The app plan's library calls were verified against Media3, Room and Compose sources at the pinned versions. Do not "fix" the plans from memory; if a step fails, record the exact error and fix minimally, then update the plan.
