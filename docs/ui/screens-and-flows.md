...
| Channels, Browse, Search, Favorites, Settings, Advanced, Diagnostics, dialogs | Compose | Compose | Compose | Compose (long-click where the screen defines one) | — | pop one level; Search with the keyboard open: close the keyboard first | Favorites | ignored | Channels | pause / rejoin |
