...
- language rule · Nothing on screen says HLS, TS, DASH, unverified, unsorted, demoted, or a stream count. Status words are exactly Working, Not checked, Not working. A stream count means a total ("5 sources"); positions within a tune ("Trying source 2 of 5") are allowed, and Diagnostics shows the format word (owner decisions 2026-09-24).
